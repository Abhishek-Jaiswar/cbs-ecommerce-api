...
const BLOG_CACHE_TTL_SECONDS = 60 * 60;

const BLOG_VERSION_FALLBACK = 1;

const BLOG_LIST_VERSION_KEY = "blog-post:list:version";

const buildPostDetailsKey = (id: string) => `blog-post:details:${id}`;

const buildPostSlugKey = (slug: string) => `blog-post:slug:${slug}`;

const buildPostListKey = (version: number, page: number, limit: number) =>
...
class BlogPostCache {
  ...
  async getOrSetPostList<T>(
    ...
    return cacheService.remember(
      buildPostListKey(version, page, limit),
  ...
  async invalidatePostLists() {
    const nextVersion = await cacheService.increment(BLOG_LIST_VERSION_KEY);
  ...
  async invalidatePost(
    ...
    const keys = [buildPostDetailsKey(id)];

    if (slug) {
      keys.push(buildPostSlugKey(slug));
    }

    const deleted = await cacheService.delete(...keys);
